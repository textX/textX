...
template <class A, class Owner>
class Attribute : public ReadOnlyAttribute<A, Owner> {
    friend Owner;
 public:
    using value_type = A;
    using ReadOnlyAttribute<A, Owner>::ReadOnlyAttribute;
    using ReadOnlyAttribute<A, Owner>::operator=; 
    using ReadOnlyAttribute<A, Owner>::operator->; 
};
...
template <class A, class Owner, size_t N>
class DynamicArrayAttribute : public ReadOnlyAttribute<std::vector<A>, Owner> {
    friend Owner;
 public:
    using dynamic_array_size_functions_type = DynamicArraySizeFunctions<N>;
    using value_type = typename ReadOnlyAttribute<std::vector<A>, Owner>::value_type;
    using inner_value_type = typename value_type::value_type;
    DynamicArrayAttribute(dynamic_array_size_functions_type _sizeFun) : ReadOnlyAttribute<std::vector<A>, Owner>{}, sizeFun(_sizeFun) { adjust_size(); }
...
template <class A, class Owner, size_t N, size_t MAX_FLAT_SIZE>
class AutomaticArrayAttribute : public ReadOnlyAttribute<std::array<A, MAX_FLAT_SIZE>, Owner> {
    friend Owner;
 public:
    using dynamic_array_size_functions_type = DynamicArraySizeFunctions<N>;
    using value_type = typename ReadOnlyAttribute<std::array<A,MAX_FLAT_SIZE>, Owner>::value_type;
    using inner_value_type = typename value_type::value_type;
    AutomaticArrayAttribute(dynamic_array_size_functions_type _sizeFun) : ReadOnlyAttribute<std::array<A,MAX_FLAT_SIZE>, Owner>{}, sizeFun(_sizeFun) { adjust_size(); }
