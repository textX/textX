...
def test_referencing_attributes():
    ...
    def ref_scope(refItem, attr, attr_ref):
        ...
        if reference is None: return Postponed()
        index = reference.refs.index(refItem)
        assert(index>=0)
        if index == 0:
            return get_referenced_object\
            (None, reference, "instance.type.vals.{}".format(attr_ref.obj_name),
             attr_ref.cls)
        else:
            return get_referenced_object\
            (None, reference.refs[index-1], "valref.type.vals.{}".format(attr_ref.obj_name),
             attr_ref.cls)
