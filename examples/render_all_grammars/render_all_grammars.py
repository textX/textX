    ...
    with open(reportfilename, "w") as md:
        ...
        for m in matches:
            ...
            outfname_base = "{}_{}".format(
                m[0].replace(path, "").lstrip(sep).replace(sep, "_"), m[1].rstrip(".tx")
            )
            ...
            with open(inname) as gr:
                for i in gr:
                    md.write("\t\t" + i)
            md.write("\n")
            rel_dest_dot_png = os.path.relpath(dest_dot_png, dirname(reportfilename))
            rel_dest_pu_png = os.path.relpath(dest_pu_png, dirname(reportfilename))
            md.write(
                f'<img width="49%" src="{rel_dest_pu_png}" alt="{rel_dest_pu_png}">\n'
            )
            md.write(
                f'<img width="49%" src="{rel_dest_dot_png}" alt="{rel_dest_dot_png}">\n'
            )
            md.write("\n\n")
